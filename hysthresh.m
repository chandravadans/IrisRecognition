...
%
% Usage: thresh_img = hysthresh(im, T1, T2)
%
% Arguments:
...
%
% Returns:
%             thresh_img  - the thresholded image (containing values 0 or 1)
%
...
function thresh_img = hysthresh(im, T1, T2)
...
thresh_img = im(:);                 % Make image into a column vector
pix = find(thresh_img > T1);        % Find indices of all pixels with value > T1
...
stack_ptr = npix;                 % set stack pointer
for k = 1:npix
    thresh_img(pix(k)) = -1;        % mark points as edges
end
...
while stack_ptr ~= 0            % While the stack is not empty
    v = stack(stack_ptr);         % Pop next index off the stack
    ...
    if v > rp1 & v < rcmr   % Prevent us from generating illegal indices
       ...
       for l = 1:8
	   ind = index(l);
	   if thresh_img(ind) > T2   % if value > T2,
	       stack_ptr = stack_ptr+1;  % push index onto the stack.
	       stack(stack_ptr) = ind;
	       thresh_img(ind) = -1; % mark this as an edge point
	   end
       end
    end
end
thresh_img = (thresh_img == -1);            % Finally zero out anything that was not an edge 
thresh_img = reshape(thresh_img,rows,cols); % and reshape the image